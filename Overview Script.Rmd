...
coin_hist <- crypto_history(coin_list, start_date="20140101")
```
...
```{r ?Transforming data?}
coin_hist_return <- as.tibble(coin_hist) %>%
  group_by(coin_hist$symbol) %>%
  tq_transmute(data = as.data.frame(coin_hist), select = coin_hist$close, mutate_fun = periodReturn, period = 'weekly', col_rename = Return)
...
weeklyReturn(coin_hist, periode = 'weekly', subset = NULL, type = 'arithmetic', leading = TRUE)
```
