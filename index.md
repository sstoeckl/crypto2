...
corrected some problems when sourcing lists that now do not have the
...
be necessary at the moment, as well as `sleep` can be set to ‘0’
seconds. If you experience strange behavior this might be due to the the
api sending back strange (old) results. In this case let `sleep = 60`
(the default) and `finalWait = TRUE` (the default).
...
![](https://s2.coinmarketcap.com/static/img/coins/64x64/1.png)![](https://s2.coinmarketcap.com/static/img/coins/64x64/2.png)![](https://s2.coinmarketcap.com/static/img/coins/64x64/3.png)
